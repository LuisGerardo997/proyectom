...
class Estado_habitacion extends CI_Controller {
  function __construct(){
    ...
    public function index(){
        if($this->session->userdata('username')){
            ...
            $this->load->view('home/habitaciones/estado_habitacion');
    ...
    public function consultar(){
        echo json_encode($this->Estado_habitacion_model->consultar());
    }

    function actualizar(){
        $selector = $this->input->post('cod_estado_habitacion');
        $cod_estado_habitacion = $selector;
        ...
        if($this->Estado_habitacion_model->actualizar($selector, $data) == true){
            echo '1';
        }
    ...
    function eliminar(){
        $idselect = $this->input->post('cod_estado_habitacion');
        $data = array(
            'estado' => '0',
        );

        if($this->Estado_habitacion_model->eliminar($idselect, $data) == true){
            echo '1';
        }
    ...
    function guardar(){
        $cod_estado_habitacion = $this->input->post('cod_estado_habitacion');
        $nombre = $this->input->post('nombre');
        $data = array(
            'cod_estado_habitacion' => $cod_estado_habitacion,
            'nombre' => $nombre,
        ...
        if($this->Estado_habitacion_model->guardar($data) == true){
            echo '1';
        }
